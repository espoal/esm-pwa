...
import { ModalAction } from '@libs/components/src/ModalAction.mjs'

export const ItemFilter = ({
  align,
  trackedItems, setTrackedItems,
  setAddLandModal
}) => {
  ...
  return (
    <>
      <div className="relative inline-flex">
        ...
        >
          <span className="ml-2 mr-2">Item Filters</span>
          ...
          <div ref={dropdown}>
              ...
              {trackedItems.map((item, index) => (
                <li className="py-1 px-3" key={index}>
                  <label className="flex items-center">
                    <input type="checkbox" className="form-checkbox" onClick={se => {selectItem(item)}} />
            ...
            <div className="py-2 px-3 border-t border-slate-200 bg-slate-50">
              <ul className="flex items-center justify-between">
                <li>
                  <button className="btn-xs bg-rose-500 hover:bg-rose-600 border-slate-200 hover:border-slate-300 text-white"
                          onClick={e => deleteSelected()}>Delete</button>
                </li>
                <li>
                  <button className="btn-xs bg-indigo-500 hover:bg-indigo-600 text-white"
                          onClick={(e) => {e.stopPropagation(); setDropdownOpen(false); setAddLandModal(true); }}
                          onBlur={() => setDropdownOpen(false)}>Add Item
                  </button>
                </li>
              </ul>
            </div>
